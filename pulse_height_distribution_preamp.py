...
for folder_info in data_folders_info:
    ...
    for file in csv_files:
        try:
            ...
            channel_b = data['Channel A']

            # Convert channel_b to numeric, ignoring non-numeric values
            channel_a_numeric = pd.to_numeric(channel_b, errors='coerce')

            # Filter out non-numeric values
            channel_a_numeric = channel_a_numeric.dropna()
            channel_a_numeric = channel_a_numeric.abs().dropna()


            # Find the maximum positive value and add to the list
            max_positive_value = channel_a_numeric[channel_a_numeric > 0].max()
...
plt.xlabel('Maximum Positive Voltage (mV)', fontsize=20)
...
plt.title('PreAmp Pulse Height Distribution for Different Data Sources at -1 V bias', fontsize=20)

# Customize legend font size and marker size
plt.legend(fontsize=20, markerscale=8)

# Save the pulse height distribution plot
plt.savefig('Preamp_pulse_height_distribution at -1 mV.png')